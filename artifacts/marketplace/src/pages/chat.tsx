            ...
            <>
              ...
              <div className="flex-1 min-w-0">
                <h2 className="font-bold text-white text-sm truncate flex items-center gap-1">
                  ...
                  {((otherParticipant as any).isVerified || (otherParticipant as any).role === "admin") && <VerifiedBadge size="xs" role={(otherParticipant as any).role} />}
                </h2>
                {typingIndicator ? (
                    ...
                    <div className="flex-1 min-w-0 cursor-pointer"
                      ...
                      <div className="flex items-baseline justify-between">
                        <span className={`font-bold text-sm flex items-center gap-1 ${hasUnread ? "text-white" : "text-white/80"}`}>
                          {other?.name || "مجهول"}
                          {(other?.isVerified || other?.role === "admin") && <VerifiedBadge size="xs" role={other?.role} />}
                        </span>
                        {conv.lastMessage && (
