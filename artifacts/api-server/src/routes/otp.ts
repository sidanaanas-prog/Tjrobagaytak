...
// ── تسجيل دخول تجريبي (لمراجعي Google Play) ──────────────────
router.post("/auth/demo/login", async (req, res): Promise<void> => {
  try {
    const DEMO_PHONE = "+966500000000";
    let [user] = (await db.select().from(usersTable).where(eq(usersTable.phone, DEMO_PHONE))) ?? [];
    ...
    if (!user) {
      ...
      await db.insert(activityTable).values({
        id: randomUUID(),
        type: "user_registered",
        description: "Demo reviewer account created",
        userId: id,
        userName: "Gaytak Demo",
      });
    }
    ...
    req.log.error({ err: e }, "demo/login error");
    res.status(500).json({ error: "حدث خطأ، حاول مجدداً" });
  }
});
