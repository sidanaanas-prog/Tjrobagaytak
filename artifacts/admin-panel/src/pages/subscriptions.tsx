...
import {
  ...
  User, Phone, Calendar, Eye, X, ChefHat,
} from "lucide-react";
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <CreditCard className="w-6 h-6 text-primary" /> طلبات الاشتراك
          </h1>
          ...
          return (
            <button
              ...
              className={`rounded-xl border p-3 text-center transition-all ${
                typeFilter === t ? "border-primary bg-primary/10" : "bg-card border-border"
              }`}
            >
              <Icon className={`w-5 h-5 mx-auto mb-1 ${info.color.split(" ")[0]}`} />
              <p className="text-xs font-bold text-foreground">{info.label}</p>
              {cnt > 0 && (
                <p className="text-[10px] text-yellow-400 mt-0.5">{cnt} معلّق</p>
              )}
            </button>
          );
        })}
