...
export default function Users() {
  ...
  const handleReactivate = async () => {
    ...
    try {
      const token = localStorage.getItem("glow_admin_token");
      const res = await fetch(`${BASE}/api/admin/users/${userToReactivate.id}/reactivate`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ months: reactivateMonths }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      toast({ title: "✅ تم إعادة التفعيل", description: `${userToReactivate.name} — تم تجديد الاشتراك لمدة ${reactivateMonths} أشهر` });
      queryClient.invalidateQueries({ queryKey: getListUsersQueryKey(queryParams) });
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {user.id !== currentUser?.id && user.role !== "admin" && (
                          <>
                            ...
                            {/* زر إعادة تفعيل الاشتراك — يظهر فقط إذا انتهى الاشتراك أو الحساب غير موثّق */}
                            {(!user.isVerified || !user.subscriptionExpiresAt || new Date(user.subscriptionExpiresAt) < new Date()) && (
                              <Button
                                size="icon" variant="ghost"
                                className="h-8 w-8 text-muted-foreground hover:text-green-400 hover:bg-green-400/10"
                                onClick={() => setUserToReactivate({ id: user.id, name: user.name, subscriptionExpiresAt: user.subscriptionExpiresAt })}
      ...
      {/* ── ديالوج إعادة تفعيل الاشتراك ── */}
      <Dialog open={!!userToReactivate} onOpenChange={(open) => !open && setUserToReactivate(null)}>
        <DialogContent className="bg-card border border-green-500/30 font-mono max-w-md">
          <DialogHeader>
            <DialogTitle className="text-green-400 uppercase tracking-wider flex items-center gap-2">
              <RefreshCw className="w-5 h-5" />
              إعادة تفعيل الاشتراك
            </DialogTitle>
            <DialogDescription className="text-muted-foreground space-y-1">
              <span className="block">المتجر: <span className="text-foreground font-bold">{userToReactivate?.name}</span></span>
              {userToReactivate?.subscriptionExpiresAt && (
                <span className="block text-xs text-destructive">
                  انتهاى الاشتراك: {new Date(userToReactivate.subscriptionExpiresAt).toLocaleDateString("ar")}
                </span>
              )}
              <span className="block text-xs text-yellow-400/70 mt-1">
          ...
          <div className="space-y-3">
            <label className="text-xs text-muted-foreground uppercase">مدة الاشتراك (بالأشهر)</label>
            <div className="flex gap-2">
              {[3, 6, 12].map((m) => (
                <button
                  key={m}
                  onClick={() => setReactivateMonths(m)}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setUserToReactivate(null)} className="border-primary/20 text-muted-foreground">
              إلغاء
            </Button>
            <Button
            ...
            >
              {reactivateLoading ? (
                <span className="flex items-center gap-2"><span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> جارٍ التفعيل...</span>
              ) : (
                <span className="flex items-center gap-2"><CreditCard className="w-4 h-4" /> تأكيد وتفعيل</span>
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
